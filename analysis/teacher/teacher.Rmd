...
```{r}
nonseq.agg.mod <- d %>%
  filter(block_type == "nonSeqFull" | block_type == "nonSeqPartial", understood == "yes", total_bonus > 0) %>%
  rename(full = block_type) %>%
  mutate(full = ifelse(full == "nonSeqFull", "full", "partial")) %>%
  lmer(
    normalized_mean ~ 1 + factor(normalized_theta) + full + (1 + factor(normalized_theta) + full | subject_id), 
    data = .
  )
...
```{r}
seq.agg.mod <- d %>%
  filter(block_type == "seqFeedback" | block_type == "seqNoFeedback", understood =="yes", total_bonus > 0) %>%
  ...
  ungroup() %>%
  lmer(
    norm_mean_diff ~ 1 + factor(normalized_theta) + feedback + (1 + factor(normalized_theta) + feedback | subject_id),
    data = .
  )
...
View means and SD for feedback case, for power analysis 
