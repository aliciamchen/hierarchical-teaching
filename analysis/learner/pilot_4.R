...
d.raw <- read.csv(here("data/learner/pilot_4/pilot_4_data.csv"))
...
d.model <- read.csv(here("model/learner/output/sa3_la3_gc0.1.csv")) %>% 
  mutate(
...
line.data <- data.frame(xintercept = c(0.9 * n.ex.teacher, 0 * n.ex.teacher,  0.3 * n.ex.teacher, 0.7 * n.ex.teacher, 1 * n.ex.teacher), lines = c("what student saw (9 heads, 1 tail)", "possible true concept", "possible true concept", "possible true concept", "possible true concept"))
