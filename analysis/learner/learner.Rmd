...
d.seq <- d %>%
  ...
  # pivot wider used to combine based on axis (ie bring first and second example together)
...
# %>%, pipes, essentially like chaining using "." in pandas/python
d.nonseq.means <- d.nonseq %>% 
  # groupby splits the data based on certain axes, ex student prior
  group_by(student_prior, teacher_knowledge) %>% 
  # tidyboot mean calculates mean and confidence intervals
  tidyboot_mean(
...
```{r}
# linear models -> look up and read about how these work
nonseq.mod <- lmer_alt(
