...
d.raw <- read.csv(here("data/learner/pilot_7/pilot_7_data.csv"))
theme_set(theme_few(base_size = 15))


d <- d.raw %>%
  filter(n_attention_passed == 2, is.finite(first_examples_a) == TRUE || is.finite(second_examples_a) == TRUE) %>%
  mutate(
...
mod <-
  glmer(
    guess ~ 1 + teacher_knowledge + (1 + teacher_knowledge |
                                          subject_id),
    data = d,
    family = 'binomial'
  )
...
powerSim(mod, test = fixed("poly(first_examples_a, 2)1"), nsim = 50)

mod1 <- extend(mod, along = "subject_id", n = 100)
powerSim(mod1, test = fixed("poly(first_examples_a, 2)1"), nsim = 100)

pwrplot <-
  powerCurve(mod1,
             test = fixed("first_examples_a"),
             along = "subject_id",
             nsim = 30)
plot(pwrplot)
