...
# python preprocess.py --in_dir pilot_2 --out_dir teacher_2d/pilot_2 --expt_label expt3
...
def main(args):
    ...
    for filename in filenames:
        ...
        cols = ['subject_id', 'student_idx', 'block_type', 'trial_num', 'trueStemThresh', 'trueCapThresh', 'trueStemDir', 'trueCapDir', 'prior',
                'stemResponse', 'capResponse', 'sliderVal', 'mushroomSelected']
        ...
        for i, trial in enumerate(data):
            ...
            if trial['rt'] != None: # if they chose a response
                if ((trial['scenario'] == 'seqFeedback') | (trial['scenario'] == 'seqNoFeedback')) & (trial['exampleSet'] == 'first'): # if it's one of the trials where you select the exact mushroom
                    df_data.loc[i, 'stemResponse'] = trial['response1']
                    df_data.loc[i, 'capResponse'] = trial['response2']
                else:
                    df_data.loc[i, 'sliderVal'] = trial['response']
                    df_data.loc[i, 'mushroomSelected'] = trial['mushroomSelected']
