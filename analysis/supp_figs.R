...
fig.nonseq.model <-
  ...
  facet_grid(~alpha, labeller = labeller(.rows = label_both, .cols = label_both)) + 
...
ggplot(data = d.model %>% filter(speaker_alpha == listener_alpha) %>% rename(alpha = speaker_alpha) %>% filter(theta == 0.3, student_a == 9, alpha != 3), aes(x = first_examples_a, y = guess)) +
  geom_point(size = 2) +
...
ggplot(data = d.model %>% filter(theta == 0.3, student_a == 9, speaker_alpha != 3), aes(x = first_examples_a, y = guess, col = listener_alpha, group = listener_alpha)) +
  geom_point(size = 2) +
  geom_path(size = 1, alpha = 0.7) + 
  geom_vline(xintercept = 0.9 * 5, color = 'blue', linetype = "dashed") + 
...
ggplot(data = d.model %>% filter(student_a == 9, speaker_alpha == 4, listener_alpha == 4, guess_cost == 0.1), aes(x = first_examples_a, y = first_guess, col = theta, group = theta)) + 
  geom_point(size = 2, alpha = 0.7) + 
  geom_path(size = 1, alpha = 0.7) + 
  geom_vline(xintercept = 0.9 * 5, color = 'blue', linetype = "dashed") + 
